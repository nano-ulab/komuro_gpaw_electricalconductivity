...
view(system)
write('model_DFT_FeC-graphene.png', system)

rank = mpi.world.rank
parallel.parprint(str(rank))
...
Ef_scat = system.calc.get_fermi_level()
parallel.parprint(Ef_scat)
parallel.parprint(type(Ef_scat))
parallel.parprint("the process rank is "+str(rank))
H_skMM, S_kMM = get_lcao_hamiltonian(system.calc)
parallel.parprint(type(H_skMM))
parallel.parprint(type(S_kMM))
